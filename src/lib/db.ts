...
export async function updateRule(env: Env, id: string, rule: Partial<Rule>): Promise<void> {
  ...
  if (rule.name !== undefined) { sets.push("name = ?"); binds.push(rule.name); }
  if (rule.instrumentId !== undefined) { sets.push("instrument_id = ?"); binds.push(rule.instrumentId); }
  if (rule.type !== undefined) { sets.push("type = ?"); binds.push(rule.type); }
  if (rule.params !== undefined) { sets.push("params = ?"); binds.push(JSON.stringify(rule.params)); }
  if (rule.notify !== undefined) { sets.push("notify = ?"); binds.push(JSON.stringify(rule.notify)); }
  if (rule.active !== undefined) { sets.push("active = ?"); binds.push(rule.active ? 1 : 0); }
  
  sets.push("updated_at = ?"); binds.push(Math.floor(Date.now() / 1000));
  binds.push(id);
  
  if (sets.length > 1) { // At least updated_at
    await env.DB.prepare(`UPDATE rules SET ${sets.join(", ")} WHERE id = ?`).bind(...binds).run();
  }
}

export async function updateRuleState(env: Env, id: string, state: any): Promise<void> {
  await env.DB.prepare(
    "UPDATE rules SET state = ? WHERE id = ?"
...
function parseRule(row: any): Rule {
  return {
    ...
    state: row.state ? (typeof row.state === 'string' ? JSON.parse(row.state) : row.state) : undefined,
